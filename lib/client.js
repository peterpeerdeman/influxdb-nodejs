...
class Client extends EventEmitter {
  ...
  /**
   * Return the write queue in influxdb line format
   * @return {Promise}
   * @since 2.2.0
   * @example
   * client.write('http')
   ...
   *   })
   *   .queue();
   * client.writeQueueLineFormat()
   *   .then((arr) => console.info('write write queue data: ' + arr))
   *   .catch(err => console.error(`sync write queue fail, ${err.message}`));
   * // => sync write queue success
   */
  writeQueueLineFormat() {
    const internalData = internal(this);
    const set = internalData.writeQueue;
    const { influx } = internalData;
    const arr = Array.from(set);
